...
begin
  ...
  select * into v_quote from public.shipping_quote(
    ...
    'FRETE30', '79002-000', 'Campo Grande', 'MS');
...
begin
  ...
  begin
    perform public.create_order(
      ...
      p_shipping_complement => null, p_shipping_neighborhood => 'Centro',
      p_shipping_city => 'Cuiabá', p_shipping_state => 'MT');
  exception when others then
...
begin
  ...
  begin
    perform public.create_order(
      ...
      p_shipping_postal_code => '79330000', p_shipping_street => 'Rua A', p_shipping_number => '1',
      p_shipping_complement => null, p_shipping_neighborhood => 'Centro',
      p_shipping_city => 'Corumbá', p_shipping_state => 'XYZ');
  exception when others then
    if sqlerrm <> 'invalid_shipping_state' then raise; end if;
    v_denied := v_denied + 1;
  end;

  if v_denied <> 3 then raise exception 'FAIL: endereco invalido aceito (% recusas de 3)', v_denied; end if;
  raise notice 'PASS - CEP curto, rua ausente e UF impossivel recusam o pedido';
end;
$t$;
