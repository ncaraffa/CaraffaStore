...
/**
 * Resultado do claim atômico — reflete o jsonb retornado por
 * `claim_recovery_grant_for_password_change` (supabase/migrations/0003_recovery_session.sql).
 * `attemptId`/`completionCapability` só vêm preenchidos quando
 * `claimed === true`. `completionCapability` NUNCA deve ser enviada ao
 * navegador nem logada — só existe em memória do processo Next.js entre
 * este claim e a chamada subsequente a `completePasswordRecoveryAttempt`
 * (módulo server-only dedicado à conclusão, em lib/supabase/service-only/),
 * feita SOMENTE depois de `supabase.auth.updateUser({password})` ter sucesso real
 * (qa/reports/TASK-002-CLAUDE-VERIFICATION-3.md, BUG-CLAUDE-VERIF3-001).
 */
export interface RecoveryClaimResult {
  claimed: boolean;
  attemptId: string | null;
  completionCapability: string | null;
}
...
/**
 * Reivindica atomicamente a tentativa de recuperação da sessão atual
 * para a troca de senha — deve ser chamada IMEDIATAMENTE ANTES de
 ...
 * — sem o cookie certo, mesmo uma sessão com uma tentativa pendente de
 * verdade não reivindica nada.
 *
 * `claimed: false` significa que não há nada a reivindicar (nonce
 * ausente ou incorreto, sem sessão de recuperação válida, ou já
 * reivindicado por outra requisição concorrente com o mesmo
 * access/refresh token — BUG-RT2-002, qa/reports/TASK-002-RETEST.md): a
 * Server Action deve recusar a troca sem chamar `updateUser()`. O UPDATE
 * condicional atômico dentro da função é o que garante que, sob duas
 * tentativas concorrentes, exatamente uma autorização seja bem-sucedida.
 *
 * Quando `claimed: true`, `attemptId`/`completionCapability` devem ser
 * guardados apenas em variáveis locais do Server Action e passados a
 * `completePasswordRecoveryAttempt` (módulo server-only dedicado à
 * conclusão, em lib/supabase/service-only/) SOMENTE depois de
 * `updateUser()` retornar sucesso real — nunca
 * incluídos no `ResetPasswordState` devolvido ao cliente
 * (qa/reports/TASK-002-CLAUDE-VERIFICATION-3.md, BUG-CLAUDE-VERIF3-001).
 */
export async function claimRecoveryGrantForPasswordChange(
  ...
  const notClaimed: RecoveryClaimResult = { claimed: false, attemptId: null, completionCapability: null };
  if (!nonce) return notClaimed;

  const { data, error } = await supabase.rpc("claim_recovery_grant_for_password_change", {
    p_nonce: nonce,
  });
  if (error || !data) return notClaimed;

  if (data.claimed !== true || !data.attempt_id || !data.completion_capability) {
    return notClaimed;
  }

  return { claimed: true, attemptId: data.attempt_id, completionCapability: data.completion_capability };
}
